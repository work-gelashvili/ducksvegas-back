...
const depositMoney = express.Router();
...
depositMoney.post("/users/balance/deposit", (req, res) => {
                  ...
                  (err, rows) => {
                    ...
                    if (data.length === 0) {
                      ...
                      pool.getConnection((err, conn) => {
                        conn.query(
                          `UPDATE users SET balance=${addBalance} WHERE userId=${userID}; INSERT INTO transactions(transactionId, referenceTransactionId, userId) VALUE('${transactionId}', '0', ${userID})`,
                          [1, 2],
                          (err, row) => {
                            conn.release();
