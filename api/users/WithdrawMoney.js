...
const withdrawMoney = express.Router();
...
withdrawMoney.post("/users/balance/withdraw", (req, res) => {
              ...
              pool.getConnection((err, con) => {
                con.query(
                  ...
                  (err, rows) => {
                    ...
                    const balanceReceived = parseFloat(data[0].balance).toFixed(
                      2
                    );
                    let checkBalance = JSON.stringify(balanceReceived);
                    checkBalance = checkBalance.replace(".", "");
                    const balanceInt = JSON.parse(checkBalance);
                    ...
                    if (data.length === 0) {
                      ...
                      const BalanceInBd = parseInt(balanceInt);
                      const requestAmount = parseInt(amount);
                      let renewBalance = BalanceInBd - requestAmount;
                      if (renewBalance < 0) {
                        const response = {
                          errorCode: 3,
                        ...
                        let newBalance = renewBalance / 100;
                        const addBalance = newBalance.toFixed(2);
                        let forResponse = JSON.stringify(renewBalance);
                        const balanceLength = forResponse.length;
                        let USDTbalance = "00000000";
                        const slicer = 8 - balanceLength;
                        USDTbalance = USDTbalance.slice(0, slicer);
                        USDTbalance = USDTbalance + forResponse;
                        pool.getConnection((err, conn) => {
                          conn.query(
                            `UPDATE users SET balance='${addBalance}' WHERE userId=${userID}; INSERT INTO transactions(transactionId, referenceTransactionId, userId) VALUES('${transactionId}', '', '${userID}')`,
                            (err, row) => {
                              conn.release();
